...
const VIRT_WIDTH: usize = 80;
const VIRT_HEIGHT: usize = 60;

const PIXEL_SCALE: usize = 4; 

pub struct GameDisplay<T> {
    screen: T,
    column_cache: [Rgb565; 240], 
    stripe_buffer: [Rgb565; 960], 
}
    ...
    T: DrawTarget<Color = Rgb565, Error = E>,
{
    pub fn new(screen: T) -> Self {
        Self {
            screen,
            column_cache: [Rgb565::BLACK; 240],
            stripe_buffer: [Rgb565::BLACK; 960],
        }
    }

    pub fn render_scene(&mut self, player: &Player, world_map: &[[i32; 24]; 24]) -> Result<(), E> {
        let f_width = VIRT_WIDTH as f32;
        let edge_tolerance: f32 = 0.05;

        for x_virt in 0..VIRT_WIDTH {
            ...
            let line_height = if perp_wall_dist > 0.01 { (60.0 / perp_wall_dist) as i32 } else { 60 };

            let mut start = -line_height / 2 + 30;
            if start < 0 { start = 0; }
            let mut end = line_height / 2 + 30;
            if end >= 60 { end = 59; }
            ...
            for y_virt in 0..VIRT_HEIGHT {
                
                let pixel_color = if (y_virt as i32) < start {
                    Rgb565::BLACK 
                } else if (y_virt as i32) >= start && (y_virt as i32) <= end {
                    if is_edge || (y_virt as i32) == start || (y_virt as i32) == end {
                        Rgb565::GREEN
                    } else {
                        match tex {
                            2 => if x_virt % 2 == 0 { Rgb565::GREEN } else { Rgb565::BLACK },
                            3 => if (x_virt ^ y_virt) & 1 == 0 { Rgb565::GREEN } else { Rgb565::BLACK },
                            4 => if (y_virt as i32) <= start + 1 || (y_virt as i32) >= end - 1 { Rgb565::GREEN } else { Rgb565::BLACK },
                            _ => Rgb565::GREEN,
                        }
                    }
                } else {
                    if (x_virt % 4 == 0) && (y_virt % 3 == 0) { Rgb565::new(0, 16, 0) } else { Rgb565::BLACK } // Floor
                };

                for offset_y in 0..PIXEL_SCALE {
                    let physical_y = (y_virt * PIXEL_SCALE) + offset_y;
                    self.column_cache[physical_y] = pixel_color;
                }
            }

            for offset_x in 0..PIXEL_SCALE {
                let start_idx = offset_x * 240;
                self.stripe_buffer[start_idx..start_idx + 240].copy_from_slice(&self.column_cache);
            }

            let current_x = x_virt * PIXEL_SCALE;
            let bounding_box = embedded_graphics::primitives::Rectangle::new(
                Point::new(0, current_x as i32),
                Size::new(240, PIXEL_SCALE as u32) 
            );
            
            self.screen.fill_contiguous(&bounding_box, self.stripe_buffer.iter().copied())?;
        }
        Ok(())
    }
}